...
(function vCardFormatter() {
	...
	module.exports = {
		...
		getFormattedString: function(vCard) {
				...
				vCard.cellPhone.forEach(
					function(number){
						if (majorVersion >= 4) {
							...
							formattedVCardString += 'TEL;TYPE=Mobile:' + e(number) + nl();
						}
					}
				);
			}
				...
				vCard.homePhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							...
							formattedVCardString += 'TEL;TYPE=Home:' + e(number) + nl();
						}
					}
				);
			}
				...
				vCard.workPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							...
							formattedVCardString += 'TEL;TYPE=Work:' + e(number) + nl();
						}
					}
				);
			}
			...
			if (vCard.customEntries) {
				if(!Array.isArray(vCard.customEntries)){
					vCard.customEntries = [vCard.customEntries];
				}
				vCard.customEntries.forEach(
					function(entry) {
						formattedVCardString += `${entry.property};TYPE=${entry.type}:` + e(entry.value) + nl();
					}
				);
			}
			...
			if (vCard.urls) {
				if(!Array.isArray(vCard.urls)){
					vCard.urls = [vCard.urls];
				}
				vCard.urls.forEach((url) => {
          formattedVCardString += 'URL'+ encodingPrefix + ':' + e(url) + nl();
			...
			if (vCard.workUrl) {
				...
				vCard.workUrl.forEach((url) => {
					formattedVCardString += 'URL;type=WORK' + encodingPrefix + ':' + e(url) + nl();
        });
			}
				...
				for (var key in vCard.socialUrls) {
					if (vCard.socialUrls.hasOwnProperty(key) && vCard.socialUrls[key]) {
						...
						if (majorVersion >= 3) {
							...
							formattedVCardString += 'item' + i + '.URL:' + vCard.socialUrls[key] + nl();
							formattedVCardString += 'item' + i + '.X-ABLabel:' + key + nl();
						}
					}
				}
			}
